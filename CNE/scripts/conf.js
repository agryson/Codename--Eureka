...
var Conf = (function(){
    ...
    */
    _INIT.saveData = function(dataIn){
        //If you don't want to break existing saves, add things to 
        //the bottom of this list and don't change existing elements
        var thingsToSave = [
                "turn",
                "mapTiles",
                "home",
        ...
        var _length = thingsToSave.length - 1;
        if(dataIn === undefined){
            var saveData = [];
            for (var i = _length; i >= 0; i--) {
                saveData.push(Conf[thingsToSave[_length - i]]);
            };
            return saveData;
        } else {
            for(var i = _length; i >= 0; i--) {
                Conf[thingsToSave[i]] = dataIn[i];
            };
